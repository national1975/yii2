...
Авторизация это процесс верификации пользователя на наличие разрешиний выполнять какие-либо действия. Yii обеспечивает два вида авторизации: Access Control Filter (ACF) и Role-Based Access Control (RBAC).


Access Control Filter
---------------------

Access Control Filter (ACF) это простой способ авторизации для приложений, которым необходим простой контроль доступа. Как следует из названия, ACF это фильтр действий, который может быть использован в контроллере или модуле посредством поведения (behavior). ACF проверяет набор [[yii\filters\AccessControl::rules|access rules]] чтобы проверить имеет ли пользователь доступ к определенным действиям.

Следующий код показывает как использовать ACF реализованный в [[yii\filters\AccessControl]]:
...
В вышепреведенном коде ACF прикрепляется к контроллеру `site` в виде поведения. Это обычный способ использования фильтра действий. Опция `only` указывает, что ACF применяется только к действиям `login`, `logout` и `signup`.
Опция `rules` устанавливает [[yii\filters\AccessRule|access rules]], которые читаются следующим образом:

- Разрешить всем Гостям (ещё не authenticated) доступ к действиям 'login' и 'signup'. Опция `roles` содержит знак вопроса `?` специальный символ обозначающий группу "guests".
